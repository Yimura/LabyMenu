...
namespace YimMenu
{
    ...
    constexpr auto CUSTOM_RANDOM_SEED = 0x99d7'f7ab'99a6'b57aULL;
    ...
    #define RAND_NEXT (CTRandomGeneratorValueSeeded<__COUNTER__>)
    // generates a random number seeded with time and the custom seed between min and max ( [min, max[ )
    #define RAND_NEXT_BETWEEN(min, max) (min + (CTRandomGeneratorValueSeeded<__COUNTER__> % (max - min)))
    // generates a random number seeded with time and the custom seed with a limit ( [0, limit[ )
    #define RAND_NEXT_LIMIT(limit) RAND_NEXT_BETWEEN(0, limit)
    ...
    const uint64_t CTRandomSeed = (CUSTOM_RANDOM_SEED ^ CTRandomTimeSeed);
    ...
    template <uint64_t n, uint64_t seed = CTRandomSeed>
